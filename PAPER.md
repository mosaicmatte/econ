...
> **Scope note.** This revision merges the original systems paper — edge CV, autonomous
> digitization, the thermodynamic engine, and the software architecture — with the
> per-room identification work: the evidence for why fixed-rule BMS control fails
> (§2), the identification mathematics (§7.7–7.11), and measured results (§8). Sections
> 3–6 and the physics of §7.1–7.6 are from the original; §2, §7.7–7.11 and §8 are new.
...
### 5.5 Reinforcement Learning Operations
